...
void Compiler::compile_stmt(ast::Stmt *stmt)
{
    ...
    if (auto *node = std::get_if<ast::Print_stmt>(&stmt->node)) visit_print_stmt(*node);
    else if (auto *node = std::get_if<ast::Expr_stmt>(&stmt->node)) visit_expr_stmt(*node);
    else if (auto *node = std::get_if<ast::Var_stmt>(&stmt->node)) visit_var_stmt(*node);
    else if (auto *node = std::get_if<ast::Block_stmt>(&stmt->node)) visit_block_stmt(*node);
    else if (auto *node = std::get_if<ast::If_stmt>(&stmt->node)) visit_if_stmt(*node);
    else if (auto *node = std::get_if<ast::While_stmt>(&stmt->node)) visit_while_stmt(*node);
    else
        std::println(stderr, "Unimplemented stmt node at index: {}", stmt->node.index());
}
...
void Compiler::compile_expr(ast::Expr *expr)
{
    ...
    if (auto *node = std::get_if<ast::Literal_expr>(&expr->node)) visit_literal_expr(*node);
    else if (auto *node = std::get_if<ast::Binary_expr>(&expr->node)) visit_binary_expr(*node);
    else if (auto *node = std::get_if<ast::Unary_expr>(&expr->node)) visit_unary_expr(*node);
    else if (auto *node = std::get_if<ast::Variable_expr>(&expr->node)) visit_variable_expr(*node);
    else if (auto *node = std::get_if<ast::Assignment_expr>(&expr->node)) visit_assignment_expr(*node);
    else
        std::println(stderr, "Unimplemented expr node at index: {}", expr->node.index());
}
...
{
    // 1. Compile the initializer if it exists (leaves value on stack)
    if (stmt.initializer) {
        compile_expr(stmt.initializer);
    } else {
        if (auto *prim = std::get_if<types::Primitive_kind>(&stmt.type)) {
            switch (*prim) {
                case types::Primitive_kind::Int:    emit_constant(Value(static_cast<int64_t>(0)), stmt.loc); break;
                case types::Primitive_kind::Float:  emit_constant(Value(0.0), stmt.loc); break;
                case types::Primitive_kind::Bool:   emit_op(Op_code::False, stmt.loc); break;
                case types::Primitive_kind::String: emit_constant(Value(std::string("")), stmt.loc); break;
                default:                            emit_op(Op_code::Nil, stmt.loc); break;
            }
        } else {
...
void Compiler::visit_if_stmt(const ast::If_stmt &stmt)
{
    ...
    if (stmt.else_branch) compile_stmt(stmt.else_branch);
...
void Compiler::visit_variable_expr(const ast::Variable_expr &expr)
{
    ...
    if (arg != -1) {
        // Local Variable - Instantly fetch from array slot
        emit_op(Op_code::Get_local, expr.loc);
        emit_byte(static_cast<uint8_t>(arg), expr.loc);
    } else {
        // Global Variable - Search Hash Map
...
void Compiler::visit_unary_expr(const ast::Unary_expr &expr)
{
    compile_expr(expr.right);
    switch (expr.op) {
        case lex::TokenType::LogicalNot: emit_op(Op_code::Not, expr.loc); break;
        case lex::TokenType::BitNot:     emit_op(Op_code::BitNot, expr.loc); break;
        case lex::TokenType::Minus:      emit_op(Op_code::Negate, expr.loc); break;
        default: break;
    }
}
