...
[[noreturn]]
inline auto todo(std::source_location loc = std::source_location::current()) -> void
{
    todo("", loc);
}
} // namespace cl::debug
...
template <typename T>
concept Supported_Scalar_C = std::is_same_v<T, bool> || std::is_same_v<T, cl::Num> || std::is_same_v<T, cl::Fp_Num>
...
template <typename T>
concept Gettable_Scalar_C =
    std::is_same_v<T, Num> || std::is_same_v<T, Fp_Num> || std::is_same_v<T, std::string> || std::is_same_v<T, bool>;

template <typename T>
concept Gettable_Type_C = Gettable_Scalar_C<T> || (is_std_vector_v<T> && Gettable_Scalar_C<typename T::value_type>)
...
constexpr auto operator*(Flags f) -> const uint16_t
{
...
{
    return static_cast<Flags>(~std::to_underlying(f));
}
constexpr Flags &operator|=(Flags &a, Flags b) noexcept
{
    return a = a | b;
...
{};
template <>
struct _opt_type_to_canonical_type_t_<Opt_type::Int>
{
    using value = long long;
};
template <>
struct _opt_type_to_canonical_type_t_<Opt_type::Bool>
{
    using value = bool;
};
template <>
struct _opt_type_to_canonical_type_t_<Opt_type::Str>
{
    using value = std::string;
};
template <>
struct _opt_type_to_canonical_type_t_<Opt_type::Float>
{
    using value = double;
};
...
public:
    ...
    auto
    add_sub_cmd(const std::string &name, const std::string &desc, uint16_t flags, Subcommand_id parent_id = global_command, int reserve = 4)
        -> Subcommand_id;
    auto parse(int argc, char *argv[]) -> std::expected<Parse_res, Parse_err>;
    auto print_help(std::ostream &os = std::cout, Subcommand_id sub_id = global_command, std::optional<Opt_id> opt_id = std::nullopt)
        -> void;
    template <typename T, typename... Configs>
...
cl::Parser::Parser(std::string s, std::string des, std::size_t reserve)
    : _name(std::move(s)), _description(std::move(des)), _schema(std::make_shared<detail::Commands_schema>())
{
...
inline auto cl::Parser::add_impl(Opt<T> opt) -> Opt_id
{
    ...
    auto validate_long_name = [&](std::string_view __name) {
        ...
        cl::asrt::t(
            valid_chars && !__name.starts_with("-"),
...
inline void cl::Parser::handle_positional_and_subcmds(cl::detail::Parse_ctx &ctx, std::string_view value)
{
    if (auto it = ctx._active_subcomamnd->child_to_id.find(value); it != ctx._active_subcomamnd->child_to_id.end()) {
        ...
        return;
    } else if (auto it = this->_schema->sub_cmds_[global_command]->child_to_id.find(value);
               it != ctx._active_subcomamnd->child_to_id.end()) {
        ctx._active_sub_id = it->second;
        ctx._active_subcomamnd = this->_schema->sub_cmds_[ctx._active_sub_id];
        ctx._res._visited_subcommands.insert(ctx._active_sub_id);
        return;
...
inline auto cl::Parser::add_sub_cmd(const std::string &name, const std::string &desc, uint16_t flags, Subcommand_id parent_id, int reserve)
    -> Subcommand_id
{
...
constexpr auto std::formatter<cl::detail::Option, char>::parse(std::format_parse_context &ctx)
{
    return ctx.begin();
}
...
auto std::formatter<cl::detail::Option, char>::format(const cl::detail::Option &o, std::format_context &ctx) const
{
    ...
    std::format_to(
        out,
        "Option{{ id={}, type={}, storage={}, arity={}, flags=0b{:b}, default_hints: {}}}",
        o.id,
        o.type,
        o.storage,
        o.arity,
...
constexpr auto std::formatter<cl::Parse_err>::parse(std::format_parse_context &ctx)
{
    return ctx.begin();
}
...
std::size_t Arg_stream::size()
{
    return args_.size() - cur_;
}
