    ...
    for (const auto &field : stmt.fields)
    {
        ...
        if (!uses_forbidden)
        {
            ...
            if (default_type && !is_compatible(field.type, *default_type))
                type_error(field.loc,
                           "Default value for field '" + field.name + "' must have type '" +
                           types::type_to_string(field.type) + "', but got '" + types::type_to_string(*default_type) + "'.");
        }
    }
}
    ...
    for (const auto &variant : stmt.variants)
    {
        ...
        if (!uses_forbidden)
        {
            ...
            if (default_type && !is_compatible(variant.type, *default_type))
                type_error(variant.loc,
                           "Default value for variant '" + variant.name + "' must have type '" +
                           types::type_to_string(variant.type) + "', but got '" + types::type_to_string(*default_type) + "'.");
        }
    }
}
...
void TypeResolver::visit(ast::Static_path_expr &expr) { resolve_expr(*expr.base); }
...
void Type_checker::check_stmt_node(ast::Function_stmt &stmt)
{
    ...
    for (const auto &p : stmt.parameters) declare(p.name, p.type, !p.is_mut, stmt.loc);
    if (stmt.body)
...
void Type_checker::check_stmt_node(ast::Union_stmt &stmt) { validate_union_defaults(stmt); }
...
void Type_checker::check_stmt_node(ast::Return_stmt &stmt)
{
    ...
    if (stmt.expression)
    {
        if (auto val_res = check_expr(*stmt.expression, current_return_type);
            ...
            type_error(stmt.loc,
                       "Return value type does not match function's return type: expected: " +
                       types::type_to_string(current_return_type.value()) + " got: " + types::type_to_string(val_res.value()));
    }
        ...
        if (!arm.is_wildcard)
        {
                 ...
                 std::holds_alternative<ast::Enum_member_expr>(arm.pattern->node)))
            {
                ...
                auto variant_it = std::find_if(union_type->variants.begin(),
                                               ...
                                               [&](const auto &v) { return v.first == variant_name; });

                if (variant_it == union_type->variants.end())
                {
                    type_error(pattern_loc,
                               "Variant '" + variant_name + "' does not exist in union '" + union_type->name + "'.");
                }
...
Result<types::Type> Type_checker::check_expr_node(ast::Call_expr &expr, std::optional<types::Type> context_type)
{
    ...
    if (auto *static_path = std::get_if<ast::Static_path_expr>(&expr.callee->node))
    {
        ...
        if (is_union(*base_type_res))
        {
            ...
            type_error(expr.loc,
                       "Union construction via '::' is not supported. Use '" + union_type->name + "{ " + variant_name +
                       ": ... }' or '.{ " + variant_name + ": ... }' instead");
            return expr.type = types::Primitive_kind::Void;
        }
        else if (is_model(*base_type_res))
        {
            ...
            {
                type_error(expr.loc,
                           std::format("Incorrect number of arguments for static method '{}'. Expected {}, but got {}.",
                                       method_name,
    ...
    if (expr.arguments.size() != signature->parameter_types.size())
    {
        type_error(
        expr.loc,
        std::format("Incorrect number of arguments. Expected {}, but got {}.", signature->parameter_types.size(), expr.arguments.size()));
    }
    else
    {
        for (size_t i = 0; i < expr.arguments.size(); ++i)
...
Result<types::Type> Type_checker::check_expr_node(ast::Cast_expr &expr, std::optional<types::Type> context_type)
{
    ...
    if (auto *target_array = std::get_if<mem::rc_ptr<types::Array_type>>(&target_type))
    {
        if ((*target_array)->element_type == types::Type(types::Primitive_kind::U8))
        {
            ...
            {
                type_error(expr.loc,
                           "Casting to 'u8[]' is only supported from 'string', integer arrays, bool arrays, or 'u8[]'.");
                return types::Primitive_kind::Void;
            }
...
Result<types::Type> Type_checker::check_expr_node(ast::Closure_expr &expr, std::optional<types::Type> context_type)
{
    ...
    begin_scope();
    for (auto const &p : expr.parameters) declare(p.name, p.type, !p.is_mut, expr.loc);
    if (expr.body)
...
Result<types::Type> Type_checker::check_expr_node(ast::Anon_model_literal_expr &expr, std::optional<types::Type> context_type)
{
    ...
    if (is_model(expected_type))
    {
        ...
        const auto &field_defaults =
        (!expected_model->name.empty() && model_data.contains(expected_model->name))
        ...
        bool has_named_fields = std::any_of(expr.fields.begin(),
                                            expr.fields.end(),
                                            [](const auto &field) { return !field.first.empty(); });

        if (has_named_fields)
        {
            bool has_positional_fields = std::any_of(expr.fields.begin(),
                                                     expr.fields.end(),
                                                     [](const auto &field) { return field.first.empty(); });
        ...
        {
            auto it = std::find_if(expr.fields.begin(),
                                   expr.fields.end(),
                                   [&](const auto &field) { return field.first == expected_field.first; });
