...
public:
    ...
    explicit Parser(std::vector<lex::Token> tokens, Compiler_context &ctx, std::string source_name = "<input>")
        : ctx_(ctx), tokens_(std::move(tokens)), source_name_(std::move(source_name))
    {}
