...
void Tree_printer::print_node(const Binary_expr &node)
{
    ...
    with_child(true, [&] {
        indent();
        print_str("Operator: " + lex::operator_token_to_string(node.op));
    });
    with_child(true, [&] {
        indent();
        print_str("Type: " + tt.to_string(node.type));
...
{
    return "(" + lex::operator_token_to_string(node.op) + " " + print_expr(node.left) + " " + print_expr(node.right) + ")";
}

std::string Sexpr_printer::print_node(const Unary_expr &node)
{
    return "(" + lex::operator_token_to_string(node.op) + " " + print_expr(node.right) + ")";
}
