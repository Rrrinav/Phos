...
std::optional<Module_id> resolve_module_context(Compiler_context &ctx, Module_id current_module_id, ast::Expr_id expr_id)
{
    ...
    if (const auto *base_var = std::get_if<ast::Variable_expr>(&node)) {
        return imported_module_id(ctx, current_module_id, base_var->name);
    }

    if (const auto *base_path = std::get_if<ast::Static_path_expr>(&node)) {
        return base_path->resolved_module;
    }
