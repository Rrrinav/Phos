...
{
    // --- Trigonometry ---
    static Value sin(Ctx &, std::span<Value> args)
    {
        return Value(std::sin(args[0].as_float()));
    }
    static Value cos(Ctx &, std::span<Value> args)
    {
        return Value(std::cos(args[0].as_float()));
    }
    static Value tan(Ctx &, std::span<Value> args)
    {
        return Value(std::tan(args[0].as_float()));
    }
    static Value asin(Ctx &, std::span<Value> args)
    {
        return Value(std::asin(args[0].as_float()));
    }
    static Value acos(Ctx &, std::span<Value> args)
    {
        return Value(std::acos(args[0].as_float()));
    }
    static Value atan(Ctx &, std::span<Value> args)
    {
        return Value(std::atan(args[0].as_float()));
    }
    static Value atan2(Ctx &, std::span<Value> args)
    {
        return Value(std::atan2(args[0].as_float(), args[1].as_float()));
    }

    // --- Hyperbolic ---
    static Value sinh(Ctx &, std::span<Value> args)
    {
        return Value(std::sinh(args[0].as_float()));
    }
    static Value cosh(Ctx &, std::span<Value> args)
    {
        return Value(std::cosh(args[0].as_float()));
    }
    static Value tanh(Ctx &, std::span<Value> args)
    {
        return Value(std::tanh(args[0].as_float()));
    }
    static Value asinh(Ctx &, std::span<Value> args)
    {
        return Value(std::asinh(args[0].as_float()));
    }
    static Value acosh(Ctx &, std::span<Value> args)
    {
        return Value(std::acosh(args[0].as_float()));
    }
    static Value atanh(Ctx &, std::span<Value> args)
    {
        return Value(std::atanh(args[0].as_float()));
    }

    // --- Exponents & Roots ---
    static Value pow(Ctx &, std::span<Value> args)
    {
        return Value(std::pow(args[0].as_float(), args[1].as_float()));
    }
    static Value sqrt(Ctx &, std::span<Value> args)
    {
        return Value(std::sqrt(args[0].as_float()));
    }
    static Value cbrt(Ctx &, std::span<Value> args)
    {
        return Value(std::cbrt(args[0].as_float()));
    }
    static Value exp(Ctx &, std::span<Value> args)
    {
        return Value(std::exp(args[0].as_float()));
    }
    static Value exp2(Ctx &, std::span<Value> args)
    {
        return Value(std::exp2(args[0].as_float()));
    }
    static Value expm1(Ctx &, std::span<Value> args)
    {
        return Value(std::expm1(args[0].as_float()));
    }
    static Value log(Ctx &, std::span<Value> args)
    {
        return Value(std::log(args[0].as_float()));
    }
    static Value log10(Ctx &, std::span<Value> args)
    {
        return Value(std::log10(args[0].as_float()));
    }
    static Value log2(Ctx &, std::span<Value> args)
    {
        return Value(std::log2(args[0].as_float()));
    }
    static Value log1p(Ctx &, std::span<Value> args)
    {
        return Value(std::log1p(args[0].as_float()));
    }
    static Value hypot(Ctx &, std::span<Value> args)
    {
        return Value(std::hypot(args[0].as_float(), args[1].as_float()));
    }
    static Value rsqrt(Ctx &, std::span<Value> args)
    {
        return Value(1.0 / std::sqrt(args[0].as_float()));
    }
    static Value fma(Ctx &, std::span<Value> args)
    {
        return Value(std::fma(args[0].as_float(), args[1].as_float(), args[2].as_float()));
    }

    // --- Rounding & Utilities ---
    static Value ceil(Ctx &, std::span<Value> args)
    {
        return Value(std::ceil(args[0].as_float()));
    }
    static Value floor(Ctx &, std::span<Value> args)
    {
        return Value(std::floor(args[0].as_float()));
    }
    static Value round(Ctx &, std::span<Value> args)
    {
        return Value(std::round(args[0].as_float()));
    }
    static Value trunc(Ctx &, std::span<Value> args)
    {
        return Value(std::trunc(args[0].as_float()));
    }
    static Value abs(Ctx &, std::span<Value> args)
    {
        return Value(std::abs(args[0].as_float()));
    }
    static Value fract(Ctx &, std::span<Value> args)
    {
        double v = args[0].as_float();
        return Value(v - std::trunc(v));
    }
    static Value sign(Ctx &, std::span<Value> args)
    {
        double v = args[0].as_float();
        if (v > 0.0) {
            return Value(1.0);
    ...
    static Value lerp_clamped(Ctx &, std::span<Value> args)
    {
        double a = args[0].as_float();
        double b = args[1].as_float();
    ...
    static Value inverse_lerp(Ctx &, std::span<Value> args)
    {
        double a = args[0].as_float();
        double b = args[1].as_float();
    ...
    static Value remap(Ctx &, std::span<Value> args)
    {
    ...
    static Value smooth_step(Ctx &, std::span<Value> args)
    {
    ...
    static Value smoother_step(Ctx &, std::span<Value> args)
    {
        double t = std::clamp((args[2].as_float() - args[0].as_float()) / (args[1].as_float() - args[0].as_float()), 0.0, 1.0);
    ...
    // --- Angle Helpers ---
    static Value degrees(Ctx &, std::span<Value> args)
    {
        return Value(args[0].as_float() * (180.0 / 3.14159265358979323846));
    }
    static Value radians(Ctx &, std::span<Value> args)
    {
        return Value(args[0].as_float() * (3.14159265358979323846 / 180.0));
    }
    static Value wrap_angle(Ctx &, std::span<Value> args)
    {
    ...
    static Value is_nan(Ctx &, std::span<Value> args)
    {
        return Value(std::isnan(args[0].as_float()));
    }
    static Value is_inf(Ctx &, std::span<Value> args)
    {
        return Value(std::isinf(args[0].as_float()));
    }
    static Value is_finite(Ctx &, std::span<Value> args)
    {
        return Value(std::isfinite(args[0].as_float()));
    }
    static Value is_normal(Ctx &, std::span<Value> args)
    {
    ...
    static Value is_nearly_equal(Ctx &, std::span<Value> args)
    {
        double a = args[0].as_float();
        double b = args[1].as_float();
    ...
    static Value is_similar(Ctx &, std::span<Value> args)
    {
        double a = args[0].as_float();
        double b = args[1].as_float();
    ...
    static Value is_similar_eps(Ctx &, std::span<Value> args)
    {
        double a = args[0].as_float();
        double b = args[1].as_float();
    ...
    static Value ulp_distance(Ctx &, std::span<Value> args)
    {
        double a = args[0].as_float();
        double b = args[1].as_float();
        if (std::isnan(a) || std::isnan(b)) {
    ...
    static Value fibonacci(Ctx &, std::span<Value> args)
    {
        int64_t n = args[0].as_int();
        if (n < 0) {
    ...
    static Value pow_int(Ctx &, std::span<Value> args)
    {
    ...
    static Value isqrt(Ctx &, std::span<Value> args)
    {
        int64_t n = args[0].as_int();
        if (n < 0) {
            return Value(static_cast<int64_t>(-1));
    ...
    static Value is_prime(Ctx &, std::span<Value> args)
    {
        int64_t n = args[0].as_int();
        if (n < 2) {
            return Value(false);
    ...
    static Value digit_sum(Ctx &, std::span<Value> args)
    {
