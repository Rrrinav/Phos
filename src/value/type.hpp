...
enum class Primitive_kind : uint8_t { Int, Float, Bool, String, Void, Any };
...
struct Closure_type
{
    ...
    bool operator==(const Closure_type &other) const
    {
        return function_type == other.function_type &&
               captured_variables == other.captured_variables;
    }
};
...
struct Model_type
{
    ...
    {
        return name == other.name && fields == other.fields && methods == other.methods;
    }
};
...
inline bool is_function(const Type& type) {
    return std::holds_alternative<std::shared_ptr<Function_type>>(type);
}

inline std::shared_ptr<Function_type> get_function_type(const Type& type) {
    return std::get<std::shared_ptr<Function_type>>(type);
}

inline bool is_closure(const Type& type) {
    return std::holds_alternative<std::shared_ptr<Closure_type>>(type);
}

inline std::shared_ptr<Closure_type> get_closure_type(const Type& type) {
    return std::get<std::shared_ptr<Closure_type>>(type);
}

inline bool is_model(const Type& type) {
    return std::holds_alternative<std::shared_ptr<Model_type>>(type);
}

inline std::shared_ptr<Model_type> get_model_type(const Type& type) {
    return std::get<std::shared_ptr<Model_type>>(type);
}
...
std::string type_to_string(const Type& type)
{
    if (is_primitive(type)) {
        ...
        for (const auto& [name, member_type] : model_type->fields) {
            result += name + ": " + type_to_string(member_type) + ", ";
        }
