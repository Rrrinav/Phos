...
Type_table::Type_table()
{
    ...
    this->t_unknown = primitive(Primitive_kind::Unknown);
...
Type_id Type_table::iterator(Type_id elem)
{
    Iter_key k{elem};
    auto it = iter_cache_.find(k);
    if (it != iter_cache_.end()) {
        return it->second;
    }
    Type_id id{static_cast<uint32_t>(types_.size())};
    types_.push_back(Type{Iterator_type{elem}});
    iter_cache_[k] = id;
    return id;
}
...
{
    return std::holds_alternative<Enum_type>(types_[id.value].data);
}
...
Type_id Type_table::get_iter_elem(Type_id id) const
{
    return std::get<Iterator_type>(types_[id.value].data).elem;
}
