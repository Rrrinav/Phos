...
struct msg
{
    ...
    {
        return msg(std::format(fmt, std::forward<Args>(args)...), std::move(phase), line, column, std::move(filename), Severity::Error);
    }

    template <typename... Args>
    static msg warning(std::string phase, size_t line, size_t column, std::string filename, std::format_string<Args...> fmt, Args &&...args)
    {
        return msg(std::format(fmt, std::forward<Args>(args)...), std::move(phase), line, column, std::move(filename), Severity::Warning);
    }
