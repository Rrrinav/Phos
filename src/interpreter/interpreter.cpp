...
{
    __TryVoid(evaluate(*stmt.expression));
    return Return_value{};
}
...
Result<Return_value> Interpreter::execute_visitor(const ast::Function_stmt &stmt)
{
    ...
    functions[stmt.name] = {func_type, mem::make_rc<ast::Function_stmt>(stmt) , environment};
...
Result<Return_value> Interpreter::execute_visitor(const ast::Union_stmt &stmt)
{
    auto union_type = mem::make_rc<types::Union_type>();
    union_type->name = stmt.name;
    for (const auto& [name, type] : stmt.variants)
    {
        union_type->variants[name] = type;
    }
    m_union_signatures[stmt.name] = union_type;
    return Return_value{};
}
...
Result<Value> Interpreter::evaluate_visitor(const ast::Field_access_expr& expr)
{
    ...
    if (is_union(object_result))
    {
        auto union_val = get_union(object_result);
        if (union_val->tag == expr.field_name)
        {
            if (union_val->value)
            {
...
Result<Value> Interpreter::evaluate_visitor(const ast::Method_call_expr& expr)
{
    ...
    // Special case for Union.has() - handle it BEFORE evaluating arguments
    if (is_union(object))
    {
        auto union_val = get_union(object);
        if (expr.method_name == "has")
        {
            ...
            if (auto* static_path = std::get_if<ast::Static_path_expr>(&arg_expr->node))
            {
                if (auto* base_var = std::get_if<ast::Variable_expr>(&static_path->base->node))
                {
                    if (base_var->name == union_val->signature->name)
                    {
                        return Value(union_val->tag == static_path->member.lexeme);
                    }
                }
            }
...
Result<Value> Interpreter::evaluate_visitor(const ast::Model_literal_expr& expr)
{
    ...
    {
        auto val_res = __Try(evaluate(*value_expr));
...
// FIX #2: Add the missing encode_utf8 function
