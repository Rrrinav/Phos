...
namespace phos
{
    ...
    Result<std::vector<std::unique_ptr<ast::Stmt>>> Parser::parse()
    {
    ...
    Result<std::optional<std::unique_ptr<ast::Stmt>>> Parser::declaration()
    {
        skip_newlines();
        if (is_at_end())
    ...
    Result<std::unique_ptr<ast::Stmt>> Parser::function_declaration()
    {
    ...
    Result<std::unique_ptr<ast::Stmt>> Parser::model_declaration()
    {
    ...
    Result<std::unique_ptr<ast::Stmt>> Parser::var_declaration()
    {
        bool is_const = match({lex::TokenType::Const});
    ...
    Result<std::unique_ptr<ast::Stmt>> Parser::statement()
    {
        skip_newlines();
        if (is_at_end())
    ...
    Result<std::unique_ptr<ast::Stmt>> Parser::print_statement(ast::Print_stream stream)
    {
    ...
    Result<std::unique_ptr<ast::Stmt>> Parser::block_statement()
    {
        std::vector<std::unique_ptr<ast::Stmt>> statements;
    ...
    Result<std::unique_ptr<ast::Stmt>> Parser::if_statement()
    {
    ...
    Result<std::unique_ptr<ast::Stmt>> Parser::while_statement()
    {
    ...
    Result<std::unique_ptr<ast::Stmt>> Parser::for_statement()
    {
    ...
    Result<std::unique_ptr<ast::Stmt>> Parser::return_statement()
    {
        size_t line = previous().line;
        size_t column = previous().column;
    ...
    Result<std::unique_ptr<ast::Stmt>> Parser::expression_statement()
    {
        auto expr_result = expression();
        if (!expr_result)
            return std::unexpected(expr_result.error());
    ...
    Result<std::unique_ptr<ast::Expr>> Parser::expression() { return assignment(); }

    Result<std::unique_ptr<ast::Expr>> Parser::assignment()
    {
    ...
    Result<std::unique_ptr<ast::Expr>> Parser::logical_or()
    {
    ...
    Result<std::unique_ptr<ast::Expr>> Parser::logical_and()
    {
    ...
    Result<std::unique_ptr<ast::Expr>> Parser::equality()
    {
    ...
    Result<std::unique_ptr<ast::Expr>> Parser::comparison()
    {
    ...
    Result<std::unique_ptr<ast::Expr>> Parser::term()
    {
    ...
    Result<std::unique_ptr<ast::Expr>> Parser::factor()
    {
    ...
    Result<std::unique_ptr<ast::Expr>> Parser::cast()
    {
    ...
    Result<std::unique_ptr<ast::Expr>> Parser::unary()
    {
    ...
    Result<std::unique_ptr<ast::Expr>> Parser::call()
    {
    ...
    Result<std::unique_ptr<ast::Expr>> Parser::primary()
    {
    ...
    Result<std::unique_ptr<ast::Expr>> Parser::parse_closure_expression()
    {
        ...
        if (peek().type == lex::TokenType::LeftBrace)
        {
            ...
            body = std::move(block_result.value());
        }
        else
        {
            ...
            auto return_stmt_node = ast::Return_stmt{std::move(expr_result.value()), expr_loc};
            auto return_stmt = std::make_unique<ast::Stmt>(ast::Stmt{std::move(return_stmt_node)});

            // 2. Create a synthetic Block_stmt to hold our new return statement
            std::vector<std::unique_ptr<ast::Stmt>> statements;
            statements.push_back(std::move(return_stmt));

            auto block_stmt_node = ast::Block_stmt{std::move(statements), {line, column}};
            body = std::make_unique<ast::Stmt>(ast::Stmt{std::move(block_stmt_node)});
        ...
        expr->node = ast::Closure_expr{.parameters = std::move(parameters),
                                       .return_type = return_type,
                                       .body = std::shared_ptr<ast::Stmt>(std::move(body)),
    ...
    Result<std::unique_ptr<ast::Expr>> Parser::parse_array_literal()
    {
        size_t line = previous().line;
        size_t column = previous().column;
    ...
    Result<std::unique_ptr<ast::Expr>> Parser::parse_model_literal(const std::string &model_name)
    {
        auto left_brace_result = consume(lex::TokenType::LeftBrace, "Expect '{' after model name");
        if (!left_brace_result)
            return std::unexpected(left_brace_result.error());
